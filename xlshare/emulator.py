...
class CXLEmulator:
    ...
    def __init__(self, 
        ...
        self.coherence_state: Dict[Tuple[int, int], str] = {}  # (pool_id, addr) -> state
        ...
        with self.pool_locks[pool_id]:
            ...
            if self.coherence_enabled:
                with self.coherence_lock:
                    self.coherence_state[(pool_id, addr)] = "invalid"
            
            return addr
    
    def read_memory(self, pool_id: int, addr: int, size: int, 
                   source_host: int = 0) -> np.ndarray:
        """
        ...
        bandwidth_gbps = (size / (1024**3)) / max(transfer_time, 1e-6)
        
        self.stats['bandwidth_samples'].append(bandwidth_gbps)
        self.stats['latency_samples'].append(latency_ns)
        
        return data
    
    def write_memory(self, pool_id: int, addr: int, data: np.ndarray,
                    source_host: int = 0) -> bool:
        """
        ...
        # Record performance metrics
        transfer_time = time.time() - start_time
        bandwidth_gbps = (len(data) / (1024**3)) / max(transfer_time, 1e-6)
    ...
    def _handle_coherence_read(self, pool_id: int, addr: int, source_host: int) -> bool:
        """
        Handle cache coherence for read operations
        ...
        with self.coherence_lock:
            key = (pool_id, addr)
            current_state = self.coherence_state.get(key, "invalid")
            
            # Simple coherence protocol simulation
            if current_state == "invalid":
                self.coherence_state[key] = "shared"
                return True  # Coherence miss
            elif current_state in ["shared", "exclusive"]:
    ...
    def _handle_coherence_write(self, pool_id: int, addr: int, source_host: int):
        """
        Handle cache coherence for write operations
        ...
        with self.coherence_lock:
            ...
            self.coherence_state[key] = "exclusive"
            
            # In real implementation, would send invalidations to other hosts
    
    def simulate_network_contention(self, severity: float = 0.1):
        """
        Simulate network congestion and bandwidth contention
        
        Args:
            severity: Contention severity (0.0 = none, 1.0 = maximum)
        """
        if random.random() < severity:
            # Add random delay to simulate contention
            delay_ns = random.randint(100, 1000)
    ...
    def benchmark_bandwidth(self, data_size_mb: int = 100, iterations: int = 10) -> Dict[str, float]:
        """
        ...
        # Warm up
        for _ in range(2):
            self.write_memory(pool_id, addr, np.frombuffer(test_data, dtype=np.uint8), source_host=0)
            self.read_memory(pool_id, addr, data_size, source_host=0)
        
        # Benchmark local read/write
        start_time = time.time()
        for _ in range(iterations):
            self.read_memory(pool_id, addr, data_size, source_host=0)
        local_read_time = (time.time() - start_time) / iterations
        results['local_read_gbps'] = (data_size / (1024**3)) / local_read_time
        
        # Benchmark remote access
        if len(self.memory_pools) > 1:
            remote_pool = 1
            remote_addr = self.allocate_memory(remote_pool, data_size)
            self.write_memory(remote_pool, remote_addr, np.frombuffer(test_data, dtype=np.uint8), source_host=1)
            
            start_time = time.time()
            for _ in range(iterations):
                self.read_memory(remote_pool, remote_addr, data_size, source_host=0)
            remote_read_time = (time.time() - start_time) / iterations
            results['remote_read_gbps'] = (data_size / (1024**3)) / remote_read_time
